...
# 1. Validates shared library scripts exist
# 2. Embeds absolute paths into hook scripts and plugin via sed
# 3. Installs the plugin to ~/.config/opencode/plugins/
#
...
# Clean stale env plugin from previous installs
if [ -f "$OPENCODE_PLUGINS_DIR/post_code_hook_env.mjs" ]; then
    log_info "Removing stale env plugin (no longer needed)"
    rm -f "$OPENCODE_PLUGINS_DIR/post_code_hook_env.mjs"
fi

# Copy the plugin with embedded absolute paths
sed \
    -e "s|__HOOK_SCRIPT_PATH__|${HOOK_SCRIPT}|g" \
    -e "s|__TASK_HOOK_SCRIPT_PATH__|${TASK_HOOK_SCRIPT}|g" \
    "$HOOK_PLUGIN" > "$OPENCODE_PLUGINS_DIR/post_code_hook_plugin.mjs"
...
echo "  Code hook:   $HOOK_SCRIPT"
echo "  Task hook:   $TASK_HOOK_SCRIPT"
echo ""
echo "Next steps:"
...
echo "  4. Session idle will trigger comprehensive validation"
echo ""
echo "To run the hooks manually:"
echo "  Code: $HOOK_SCRIPT"
echo "  Task: $TASK_HOOK_SCRIPT"
echo ""
