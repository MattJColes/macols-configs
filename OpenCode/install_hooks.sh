...
INSTALLED_PLUGIN="$OPENCODE_PLUGINS_DIR/post_code_hook_plugin.mjs"
LEGACY_ENV_PLUGIN="$OPENCODE_PLUGINS_DIR/post_code_hook_env.mjs"

mkdir -p "$OPENCODE_PLUGINS_DIR"

# Clean existing plugin for a fresh install
if [ -f "$INSTALLED_PLUGIN" ]; then
    log_info "Clearing existing hook plugin"
    rm -f "$INSTALLED_PLUGIN"
fi

# Bake the absolute hook script path into the plugin at install time.
# shell.env only affects shells OpenCode spawns, not the plugin runtime,
# so process.env lookup in the plugin does not work — substitute instead.
sed "s|__OPENCODE_HOOK_SCRIPT__|$HOOK_SCRIPT|g" "$HOOK_PLUGIN" > "$INSTALLED_PLUGIN"
log_success "Plugin installed to $INSTALLED_PLUGIN"

# Remove the legacy env plugin if a previous install left it behind.
if [ -f "$LEGACY_ENV_PLUGIN" ]; then
    rm -f "$LEGACY_ENV_PLUGIN"
    log_info "Removed legacy $LEGACY_ENV_PLUGIN"
fi
